...
> 마지막 업데이트: **2026-09-03** (채팅 답변 품질 사고 대응 · ETF 구성종목 · 알림 진단)
> 모델: claude-opus-4-8
...
## 2026-09-23 — 느림의 원인은 MTU · 캐시 상한

### 1. "앱이 2분째 안 뜬다" → 앱은 3ms 였다
세 지점에서 같은 요청을 재는 것으로 한 번에 갈렸다: 서버 내부 0.003s / 오리진 직접 0.03s /
Cloudflare 경유 0.2~60s. nginx 로그 시각과 클라이언트 수신 시각을 대조하니 오리진이 응답을
끝낸 뒤 9초가 더 걸렸다. `ss -ti` 로 CF 연결을 보니 보낸 54KB 중 17.6KB(33%) 재전송·cwnd 4.
`ping -M do` 이진 탐색으로 경로 MTU 1500 확인(인터페이스는 OCI 기본 9000).
→ MTU 1500 고정 후 최악 30s → 1.7s. 상세는 docs/deployment.md §8.

**왜 앱을 의심하지 않았나** — 서버가 *직접 연* 연결(CF 에서 5MB 다운, 3MB 업)은 재전송 0 이었다.
같은 호스트의 같은 NIC 인데 방향에 따라 다르면 앱일 수 없다.

### 2. ttl_cache 가 항목을 지우지 않았다
TTL 은 '신선한가'만 판단하고 dict 에서 빼지 않는다. 키에 종목·인자가 들어가므로 종목 수만큼
무한히 쌓여 8일에 1.4GB(RSS 445MB + 스왑 985MB). MemoryHigh=700M 이 RSS 를 누르니
초과분이 전부 스왑으로 갔고, 스왑은 1GB 가 100% 찼다.
→ 상한 1200 + 삽입 100 회마다 만료분 청소. 좁히면 적중률이 떨어져 외부 호출이 느는 것이
   더 나쁘므로(워커 증설 금지와 같은 이유) 상한은 넉넉히 잡았다.
   `_ai_cache`(500)는 DB(ai_cache 테이블)가 정본이라 메모리를 잘라도 데이터가 남는다.
   `vm.swappiness=10` — 커널이 앱보다 페이지 캐시를 먼저 버리게 한다.


## 2026-09-16 — 레이더 2주차 · 배당 '예정' 결함 · 한 손 터치 결함 3건

**2026-09-23 배포 완료** (`index-B4LeyTaf.js` · main.py sha 일치 · guard:all 5종 통과).
배당 '예정'에 지난 날짜 0건, 하단 겹침 0 을 운영에서 실측했다.

### 결정과 근거

**1. 배당 '예정'은 오늘 이후만 (`_upcoming_dividend`)**
yfinance `calendar['Ex-Dividend Date']` 는 다음이 아니라 최근 배당락일인 경우가 많다
(실측: 005930 06-29 · AAPL 08-10 · KO 09-15 · O 09-30). 서버가 날짜를 거르지 않아
'예정' 탭에 지난 날짜가 뜨고, 타임라인 분기 막대에서 **받은 배당 + 같은 분기 '예상'** 으로
이중 합산됐다. 필터는 서버에 둔다 — `upcoming` 을 읽는 곳이 목록·막대 두 군데라서.
1회 금액은 `연간 ÷ 4` 고정 → 최근 1년 실제 지급 횟수(없으면 4). 연 1회 배당 종목이 1/4 로 보였다.
`by_ticker.ex_date` 는 그대로 두고 화면이 '다음/최근'을 구분해 적는다. `is_kr` 을 내려
한국 종목의 '$' 표기도 고쳤다.
발견 경로가 교훈이다 — 안쪽 점검이 아니라 **바깥 조사에서 푸시 비용을 재다가** 나왔다.
`guard:numbers` 는 탭 간 일치만 보므로 이런 계산 오류를 원리상 못 잡는다.

**2. 한 손 시나리오 결함 3건**
- `.app-main` 하단 여백 72 → 126px: 의견 버튼(bottom 84, 높이 34)이 마지막 행 버튼을 덮었다.
  fb-fab 위치를 바꾸면 이 값도 같이 바꿔야 한다(주석으로 묶어 둠).
- 관심 행 액션 간격 `max(2px, var(--tap-act-gap))`: 그룹 변경 바로 밑이 확인 없는 삭제다.
- 실적 캘린더 ‹ › 에 `tap-target`: `button.mono-pill` 은 세로만 44px 로 넓혀 폭이 18px 였다.
측정 함정 — elementFromPoint 가 조상을 맞혀도 명중으로 치면 터치 영역이 부풀어 보인다.

**3. 레이더 채택** — 실적발표 전날 푸시 · 급등락 푸시 관련 뉴스 1줄 · 레버리지/인버스 비중
(복권형 편향 진단 하위). 배당락 푸시는 한국·ETF 소스 부재로 분리 보류(OpenDART 후보).
상세는 `docs/RADAR.md` 2026-09-16.


## 2026-09-09 ~ 09-15 — 지인 피드백 · 자가발전 루프 · 실명 · 숫자 규약 · 발굴탭

커밋 `f0db12a` ~ `7ec9ebc` (9건). 운영 배포·실측 검증 완료.

### 왜 이 주가 중요했나
지인이 첫 사용 30분 만에 10건을 지적했다. **전부 맞았고, 전부 먼저 찾을 수 있었다.**
새 기술이 필요한 게 아니라 "처음 쓰는 사람의 눈"이 없었다. 이후 오너 지적까지 합쳐
보면 결함의 절반은 "기능이 없다"가 아니라 **"있는데 닿지 않는다 / 다른 것처럼 보인다"**였다.

### 결정과 근거

**1. 자가발전 루프를 두 짝으로 (daon-radar)**
바깥(논문·앱·기사)만 보면 남의 기능을 베끼는 목록이 쌓이고, 안(기계 점검)만 보면
있는 것을 다듬기만 한다. 채택은 **주당 3건 상한** — 상한이 없으면 백로그만 는다.
"다온의 어떤 화면/함수가 바뀌나"를 못 쓰면 기각하고, 기각도 이유와 재검토 조건을 남긴다.
검사기에는 `--selftest` 를 붙였다 — 규칙이 죽은 검사기는 초록불을 켜며 결함을 통과시킨다.
실제로 첫 자기검사에서 화면 밖 규칙이 죽어 있었다(두 겹: 문서 스크롤 판정 오류 + 재현 CSS 오류).

**2. 실명 제거 — 프런트만으로는 안 고쳐졌다**
가입 폼이 '이름 / 홍길동'을 받아 `users.name` 에 실명이 쌓였고 상단바가 그걸 띄웠다.
프런트를 닉네임만 보게 고쳤는데 **서버 세션이 `nickname or name` 으로 실명을 대신 실어**
보내고 있었다. 오너 계정은 닉네임이 있어 고쳐진 것처럼 보였고, 닉네임 없는 동료 5명은
그대로였다. → 대체 차단 + DB 실명 11건 삭제(파일 백업 + settings 스냅샷으로 되돌릴 길 확보).

**3. 탭별 총자산 불일치 — 계산이 아니라 이름**
분석 탭 1억 2,866만 vs 포트폴리오 1억 3,472만. 차이 604만 = 예수금.
`net_worth_snapshots` 는 예수금을 저장한 적이 없는데 카드명이 '자산 추이'였다.
과거에 오늘의 현금을 소급해 넣지 않았다 — 없던 사실을 지어내는 것이다.
카드명을 '주식 평가액 추이'로 바꾸고 "+ 예수금 = 총자산" 병기, `cash_krw` 는 오늘부터 누적.
**숫자 규약**을 CLAUDE.md 에 명문화하고 `number-consistency.mjs` 로 탭 간 일치를 검사한다.
검사기는 "계산이 맞나"가 아니라 "같은 이름의 숫자가 화면마다 같은가"를 본다 —
사용자는 계산을 검산하지 않고 두 화면을 나란히 볼 뿐이다.

**4. 동시 접속 — workers 가 아니라 스레드풀**
엔드포인트 320개가 전부 동기라 블로킹 I/O 가 anyio 기본 슬롯 40 을 붙잡는다 → 128.
workers=2 는 기각: `_demo_ai_calls` 가 프로세스 지역이라 데모 상한이 조용히 배가 되고,
`ttl_cache` 적중률이 반토막 나 yfinance 호출이 오히려 는다.

**5. 분석 매일 갱신 — 전략만**
전략 리포트 $0.06/회(매일 월 $1.8) vs 종목 분석 $0.08×25(매일 월 $60).
펀더멘털은 하루에 안 바뀌고 시세·비중은 매일 바뀐다. `max_users=20` 상한.

**6. 전략 분석이 탭 이동으로 멈추던 것 — 토큰도 새고 있었다**
폴링이 컴포넌트 안에 있어 언마운트되면 결과를 받을 곳이 사라졌다. 서버는 계속 만드는데
화면만 잊으니, 다시 누르면 끝난 작업을 또 시켜 토큰을 두 번 쓴다 → `utils/strategyJob.js`.

**7. API 크레딧 — 지갑이 둘이다**
오너가 claude.ai 구독 사용 크레딧($250)을 충전했지만 다온은 console API 지갑(-$0.17)을 쓴다.
AI 가 며칠 멈췄고, 그동안 사용자 화면에 영문 JSON 원문이 "형식 오류"로 노출됐다.
→ 400 을 갈래로 나눠 한국어 안내, 원문은 서버 로그로만.

### 검사기가 틀린 횟수가 앱이 틀린 횟수보다 많았다
기간 최고치를 총액으로 비교 / 계좌 카드를 히어로로 읽음 / 구분선이 단축키 번호를 먹음 /
'세액공제 한도 소진'을 오류로 판정 / 배포 도중 실행해 옛·새 번들이 섞임.
**실패가 뜨면 라벨과 요소부터 의심한다.**

## 🔴 2026-09-03 — 채팅 답변 품질 사고 (잘림 + 마크업 노출)

### 무슨 일이 있었나

종목 분석에 질문한 답이 이렇게 나갔다.

```
*결론: 기술적 신호는 유효할 수 있지만 … 선택입니다.**
…
*3. 매집 전략이 통하는          ← 문장 중간에서 끊김
```

**두 개의 독립된 결함이 한 화면에 겹쳤다.** 둘 다 기본적인 실수다.

### 원인 ① — 답이 잘렸는데 아무도 몰랐다

`max_tokens: 2000`. 한국어는 이 토크나이저에서 글자당 토큰이 커 2,000이면
1,000자 안팎이다. 넘으면 문장 중간에서 끊긴다.

**더 나쁜 것은 그걸 감지하지 않은 것이다.** 끊긴 답을 '완성된 답'처럼 저장하고
화면에도 그대로 뿌렸다. 사용자는 잘린 줄 모르고 그 내용을 믿는다.

- `max_tokens` 2000 → **4000** (상한일 뿐 생성한 만큼만 과금)
- `message_delta.stop_reason` 을 받아 `done` 이벤트에 `truncated` 로 실어 보낸다
- 화면에 "길이 제한으로 여기서 끊겼습니다 + [이어서 받기]" 를 띄운다
- 프롬프트에 "700자 이내로, 마지막 문장은 반드시 완결" 을 넣어 애초에 덜 걸리게 한다

### 원인 ② — 읽기 좋게 만들려던 분해가 원문을 망가뜨렸다

`splitAnswer()` 안에 버그가 둘이었다.

1. **선행 별표 두 개를 목록 기호로 오해했다.** `replace(/^[-•*]\s*/…)` 가
   `**결론…` 의 앞 별표를 **하나만** 지워 `*결론…` 이 남았다. 짝이 깨지니
   강조 렌더러가 매칭에 실패하고 별표가 그대로 찍혔다.
2. **긴 줄을 문장 단위로 쪼개며 `**…**` 짝을 갈랐다.** 강조가 문장 여러 개를
   감싸면 여는 표시와 닫는 표시가 서로 다른 항목으로 흩어진다.

→ `frontend/src/utils/answerText.js` 로 분리하고 규칙을 바꿨다.
별표가 2개 이상이면 마크업이므로 건드리지 않는다. 쪼갠 결과가 **짝이 맞을 때만**
쪼갠다. **읽기 편하자고 한 분해가 글을 망가뜨리면 안 된다.**

### 영구 조치

- `scripts/answer-split-check.mjs` — 7개 케이스 + 불변식("원문 짝이 맞으면 결과도
  맞는다"). **CI 프론트엔드 잡에 넣어 실패하면 빌드가 깨진다.**
  사람이 매번 눈으로 확인할 수 있는 종류가 아니라 기계가 잡아야 한다.
- CLAUDE.md 필수 확인 4번에 두 규칙 추가(마크업 보존 · 잘림 은폐 금지).

**검증(운영)**: 화면에 남은 별표 **0개** · `<strong>` 6곳 정상 렌더 ·
답변이 완결된 문장으로 끝남 · pageerror 0.

### 함께 고친 것

**ETF 구성종목 Top10** — ETF를 개별 종목처럼 PER·실적만 나열하면 분석이 공허하다.
`funds_data.top_holdings` 로 상위 10개와 비중(%)·합계를 보여준다(막대 + 숫자).
운영 확인: SOXX 상위 10개 합계 60.5%, QQQ 46.3%, NVDA는 필드 없음(개별 종목).
채팅 맥락에도 실어 ETF를 물으면 구성까지 보고 답한다.
⚠ 한국 ETF는 야후가 펀드 데이터를 주지 않는다 — 지어내지 않고 카드를 숨긴다.

**휴대폰 알림이 안 켜지던 문제** — "눌러도 꺼짐→켜짐으로 안 바뀐다".
서버는 정상이었다(VAPID 키·push-sw.js 모두 응답). 기기 쪽 원인 후보가 둘이다.
1. **`iosBlocked` 면 버튼을 `disabled` 로 막고 있었다.** 홈 화면 앱으로 열어도
   standalone 판정이 빗나가면 설치했는데도 버튼이 죽는다.
   → 판정으로 막지 않는다. 눌러보게 하고 실패하면 진짜 이유를 말한다.
2. **`navigator.serviceWorker.ready` 가 영원히 대기할 수 있다** — 에러도 없이
   버튼만 멎는다. → 8초 제한을 걸고 `sw_timeout` 으로 이름 붙여 던진다.
- 실패 사유를 7종으로 분리해 각각 다음 행동을 적었다(차단/닫음/SW응답없음/키실패/
  구독실패/저장실패/미지원).
- **진단 패널** 추가 — 지원·권한·홈화면앱·서비스워커·구독 상태를 그대로 보여준다.
  원격에서 "안 돼요"만으로는 원인을 좁힐 수 없다.

---

## 2026-08-29 — 채팅 성능 결함 · 주 1회 자동 갱신 · 로드맵 정리

### 🔴 "다온에게 물으니 다운됐다" — 재현하고 원인을 찾았다

데모(13종목)에서는 멀쩡했다. 오너 계정은 **46종목**이다. 컨텍스트 조립 시간을 재보니
**매 질문마다 15~16.6초**. 질문할 때마다 46종목 시세를 새로 긁고 있었고,
폐지된 티커(404610)가 야후 `.KS`/`.KQ` 를 매번 재시도했다. 사용자에게는
'답이 안 나온다' 로 보인다.

**두 가지를 같이 고쳤다.**

1. **캐시에 있는 시세만 쓴다**(`_cached_prices_only`). 5분 워밍 cron 이 이미 채워
   두므로 대개 즉시 끝난다. 적중률이 60% 미만이면 **6초 상한**으로만 더 기다린다 —
   늦게라도 다 채우는 것보다 빨리 답하고 없는 종목은 '모른다'고 말하는 편이 낫다.
2. **캐시 경계를 바로잡았다.** 시세를 캐시 블록에 넣고 있었는데, 프롬프트 캐싱은
   접두 일치라 **가격이 1원만 움직여도 캐시가 통째로 무효화**된다. 즉 앞서 "캐싱
   확인" 했던 것은 두 턴이 몇 초 간격이라 가격이 같았던 덕이었다.
   → 안 바뀌는 것(보유 종목·수량·평단·리포트)만 캐시하고, 시세·평가액은
   질문 바로 앞(캐시 경계 뒤)에 붙인다.

**실측**: 안정 블록 0.01초 · 시세 블록 0.38초 · 리포트 맥락 0.01초.
엔드포인트 첫 바이트 3.6초(데모 기준).

### 주 1회 자동 갱신 (오너 요청 — 이전 결정을 뒤집는다)

2026-07 에 비용 때문에 서버 cron 기반 분석 갱신을 걷어냈었다. 오너가 "토큰 여유가
있으니 주 1회 최신으로" 요청해 되살린다. **예전에 문제가 됐던 이유는 비용 자체가
아니라 '얼마나 도는지 몰랐던 것'** 이라, 상한을 분명히 두고 되살렸다.

- `POST /api/cron/refresh_strategy` 신설 — ai_enabled 승인 사용자당 1회, 데모 제외.
  화면 없이 도는 경로라 holdings/prices 를 서버가 조립한다.
- 기존 `refresh_holdings_analysis` 를 같은 스크립트에서 호출(1회 25종목, 6일 경과분만).
- `daon-weekly-refresh.sh` · 매주 **월요일 06:00 KST**(일 21:00 UTC).

**검증 중 두 개가 실제로 깨져 있었다.**
- 전략 갱신이 `'float' object has no attribute 'get'` 로 실패했다. 소비부가
  `prices[t]['current_price']` 를 기대하는데 cron 만 float 을 넣고 있었다.
  **화면 경로와 다른 모양을 쓰면 이렇게 조용히 갈라진다.** → 같은 모양으로 수정.
- 한국 펀드형 코드(447770·472170)는 AI 응답이 매번 JSON 형식을 못 맞춰 502 다.
  쿨다운이 없으면 매주 같은 종목에 돈을 쓰고, `max_tickers` 상한을 실패로 다 채워
  정상 종목이 밀린다. → **실패 시 14일 쿨다운**(`settings` 에 JSON 한 줄, 스키마 변경 없음).

검증: 전략 갱신 `refreshed: 1`(32.9초) · 종목 갱신 `refreshed: 2, failed: []` ·
쿨다운 기록/만료청소 단위 확인.

### 채팅 다듬기

- **강조**: 시스템 프롬프트에 "핵심 어구 하나를 `**`로 감싸라"를 넣고, 답변을
  `ReportBullets`(리포트와 같은 강조·숫자 색 렌더러)로 그린다. 실측 응답에
  `**+796.1%**` 확인.
- **묻기 버튼 위치**: 우하단 FAB 이 장 시작/종료 배너를 가렸다. 스크롤을 따라가게
  만들 수도 있지만 그러면 필요할 때 화면 밖에 있다. → 앱 모드는 **상단 컨트롤 줄**
  (알림·테마 옆)로 옮기고, 여백이 있는 웹 모드만 FAB 을 남겼다.
- 웹 검색 체크박스 라벨을 "웹에서 최신 정보 찾기"로 바꾸고 언제 켜는지 툴팁에 적었다.

### 로드맵 — 기록이 아니라 다음 행동

완료·보류·진행이 한 목록에 섞여 있어 "다음에 뭘 해야 하나"를 매번 눈으로 걸러야 했다.
→ 진행할 것만 **우선순위 순(High→Medium→Low)** 으로 본문에 세우고, 완료 6건은
아래 '완료 이력'으로 접었다(버리지 않는다 — 같은 것을 두 번 검토하지 않게 해준다).
같은 등급 안에서는 오너가 적어둔 원래 순서를 지킨다.

⚠ 정렬 헬퍼를 `ROADMAP` 정의보다 **앞에** 뒀다가 TDZ 로 깨질 뻔했다(핸드오프 함정과
같은 유형). 정의 뒤로 옮겼다.

---

## 2026-08-28 (4) — 다온 채팅 (맥락을 아는 대화)

### 왜 앱 안에 넣나

궁금증이 생기면 결국 외부 챗봇으로 나갔다. 거기서는 ① 포트폴리오를 매번 말로
설명해야 하고 ② 대화가 사라져 지식이 안 쌓인다. 앱 안에서는 보유·비중·지금 보던
리포트가 **서버에서** 프롬프트에 실린다. 모델 성능 차이가 아니라 구조 차이다.

### 설계 원칙 셋 — 이걸 어기면 만들 이유가 없다
...
1. **숫자는 앱이 계산하고, 모델은 설명·연결만 한다.**
   ...
   시스템 프롬프트에 "제공된 데이터에 없는 수치를 지어내지 말 것" 을 못 박았다.
2. **컨텍스트는 서버가 조립한다.** 프론트는 질문만 보낸다.
3. **대화는 `chat_messages`(user_id 스코프)에만.** 공유 캐시 금지.

### 데모에서 사고가 날 뻔했다

구현 직후 검증하다 깨달았다 — **데모는 여러 방문자가 같이 쓰는 공용 계정이다.**
대화를 저장하면 앞사람의 질문이 다음 방문자에게 그대로 보인다. 2026-08-26 자산
노출과 정확히 같은 종류다. 데모는 저장도 조회도 하지 않게 막고(브라우저 안에서만
대화가 이어진다), 검증하며 쌓인 10행을 지웠다.

### 비용 — 실측

| 항목 | 값 |
|---|---|
| 캐시 접두(시스템+보유 스냅샷) | 1,348 토큰 |
| 2번째 턴 | `cache_read 1,348 / cache_write 0` → **캐싱 확인** |
| 턴당 실측 | 입력 326 + 캐시읽기 1,348 + 출력 55 ≈ **$0.0015(2원)** |
| 긴 답변 기준 추정 | 약 $0.008(11원) → 하루 10턴이면 월 3,300원 |

- 웹 검색은 **기본 끔**. 1,000회당 $10(회당 14원)으로 토큰비와 맞먹는다.
- 월 쿼터 200턴. 종목분석과 달리 채팅은 자유 입력이라 캐시가 자연 상한이 안 된다.
- 사용량을 `audit_log`(`ai_chat`)에 남긴다. **`cache_read` 가 계속 0 이면 캐시가
  깨진 것** — 조용히 비용만 두 배가 되는 종류라 숫자를 안 남기면 못 알아챈다.
- ⚠ 보유 목록 정렬을 고정했다. 순서가 흔들리면 접두가 달라져 캐시가 통째로 날아간다.

### 가림 모드에서 한 번 반쯤 뚫렸다

응답에 금액이 그대로 들어오므로 프론트에서 `maskText` 를 통과시킨다. 그런데 운영
검증에서 **`••4천원`** 이 나왔다 — AI 가 쓰는 한국식 복합 표기(`6,148만 4천원`)를
앞부분만 먹고 꼬리가 남은 것이다. 패턴을 보강했다(퍼센트·연도는 그대로 유지).

### 진입점

채팅 버튼은 어느 화면에서든 우하단 고정. 분석 리포트 아래 "이 리포트에 대해 묻기",
종목 분석 아래 "이 분석에 대해 묻기" — 그 맥락으로 열린다. 웹은 우측 패널,
앱은 하단 시트(같은 컴포넌트, CSS 로만 분기 — 두 벌이면 한쪽만 고쳐진다).

**검증**: 운영에서 실제 대화 완주(스트리밍 4조각, 보유 데이터 기반으로 반도체 비중
43% 를 정확히 계산) · 캐싱 확인 · 데모 비저장 확인(DB 0행) · 가림 시 노출 0건 ·
pageerror 0 · pytest 209 통과.

---

## 2026-08-28 (3) — Sonnet 5 이관 (분석 모델 단가 31% 인하)

채팅창 도입 비용을 따지다 발견한 것: 종목분석·전략리포트가 **Sonnet 4.6**($3/$15)을
쓰고 있었는데, **Sonnet 5 는 $2/$10 으로 33% 싸면서 지식 컷오프는 더 최신**이다.
채팅과 무관하게 지금 바꾸는 게 순이득이라 먼저 분리해서 넣었다.

**실측으로 한 번 뒤집혔다.** 문서에는 "4.7 세대부터 토크나이저가 바뀌어 같은 텍스트가
약 30% 더 많은 토큰" 이라고 되어 있어 실질 절감이 13% 로 줄어들 거라 봤다. 그런데
우리 종목분석 프롬프트(3,093자)로 `count_tokens` 를 재보니 **2,727 → 2,822 토큰,
+3.5%** 에 그쳤다. 한국어 비중이 높아서로 보인다. → **실질 절감 약 31%.**
짧은 영문 프롬프트에서는 75 vs 50 토큰(1.5배)까지 벌어졌으니, **일반 수치를 그대로
믿지 말고 자기 프롬프트로 재는 것이 맞다.**

- 모델 ID 를 `MODEL_ANALYSIS` / `MODEL_LIGHT` 상수로 모았다. 예전에는 sonnet 이 두 곳,
  haiku 가 네 곳에 하드코딩돼 "지금 뭘 쓰는지" 를 grep 해야 알 수 있었다.
- ⚠ Sonnet 5 는 `temperature`/`top_p`/`top_k`/`budget_tokens` 를 받으면 400 이다.
  현재 `_call_claude` 는 이들을 보내지 않아 이관이 깨질 지점은 없었다 — 추가 금지.
- `max_tokens` 는 여유를 뒀다(8000→10000, 7000→9000). 상한일 뿐 생성한 만큼만 과금된다.

**검증**: 모델 ID 실호출 2건(sonnet-5 · haiku-4-5 모두 200) + 운영에서 실제 종목분석
1건 완주(PLTR — 웹검색 포함, JSON 파싱 성공, 목표가·강세/리스크·출처 정상). pytest 209 통과.

---

## 2026-08-28 (2) — 세션 기준 시세 · 기준선 · 별표 · 버전 확인

### 시세를 '지금 시간대' 기준으로 (오너 요청)

앞선 작업은 프리장 값을 **보조 표시**로만 붙였다. 오너 요청은 더 근본적이었다 —
"프리장 시간이면 당연히 그 시간대 수치를 보여줘야 한다."

바꾼 것: `effPrice()` 가 확장시간 체결가를 우선한다. 이 함수 하나가 평가액·총자산·
손익의 입력이라, 여기만 고치면 화면 전체가 세션을 따라간다.
등락률은 `sessionChange()` 가 세션에 맞는 값과 **라벨**을 함께 돌려준다.

**부수 효과 — 화면이 살아났다.** "총액만 30초마다 바뀌고 나머지는 무미건조하다"는
지적의 원인이 여기였다. 프리장 내내 현재가가 '어제 종가' 로 고정돼 있으니
FlashPrice 가 감지할 변화 자체가 없었던 것이다. 이제 프리장에도 값이 움직이며
기존 플래시 애니메이션이 그대로 작동한다.

**국장은 세션 라벨만 붙인다.** 시간외 단일가는 야후에 없고 네이버 스크래핑을 하나
더 늘리면 깨질 곳이 는다. **없는 데이터를 지어내는 대신 "이 값은 종가다" 라고 밝히는
편이 정확하다.** KR 행은 `오늘 +4.89%`, US 프리장 행은 `프리 +2.10%` 로 표시된다.

평가액 모드에는 일간 변동이 아예 없어서 "지금 오르는 중인지" 를 알 수 없었다 —
수량·계좌 옆에 `오늘/프리 +2.1%` 를 작게 상시 표시한다(평가손익과 헷갈리지 않게
라벨을 앞에 둔다).

### 추이선 기준선 (오너 제안)

스파크라인은 색으로만 방향을 알렸다 — 색약·작은 화면에서 약하다.
**직전 정규장 종가에 회색 점선**을 그어 기준을 만들었다. 선이 점선 위면 상승,
아래면 하락. 기준값은 백엔드를 건드리지 않고 역산한다(`prevClose()`):
확장시간이면 `current_price` 가 곧 기준이고, 정규장이면 `cur / (1 + pct/100)`.
기준선이 표시 구간 밖이면 그리지 않는다 — 가장자리에 붙은 선은 기준선이 아니라
테두리처럼 보여 오히려 방향을 헷갈리게 한다.

### 관심 별표 — 담고 싶은 자리에서 담는다

예전에는 관심 탭으로 가서 **다시 검색해야** 담을 수 있었다. 담고 싶은 순간과 담는
장소가 떨어져 있으면 사람은 그냥 안 담는다. `WatchStar` 를 종목 상세·시장·발굴에
붙였다(운영 실측: 시장 10개 · 발굴 36개, 토글 왕복 동작 확인).
⚠ 목록 행 안에 놓이므로 `stopPropagation` 필수 — 아니면 별을 눌렀는데 차트로 이동한다.

### "고쳤다는데 내 폰은 그대로" (오너 질문)

매니페스트·SW·캐시 헤더 모두 정상이었고 `skipWaiting` 도 걸려 있었다. 문제는
**홈 화면 앱이 완전히 닫히기 전까지 이전 화면을 들고 있다**는 것, 그리고
**지금 무슨 버전을 보는지 알 방법이 없다**는 것이었다.
설정 탭에 `앱 버전 v2.5` + **업데이트 확인** 버튼을 넣었다(서비스워커 update → 새로고침).
변경 이력 v2.5 도 추가했다 — 폰에서 이 모달이 뜨면 그 자체가 갱신 확인이 된다.

---

## 2026-08-28 — 프리마켓 지원 · 리포트 표현 통일 · PWA 설치 안내

### 프리마켓/애프터마켓 (오너 지적 ④ — 실제 오류였다)

프리장(ET 09:04)에 NVDA 가 **+8.7%** 로 떠 있었다. 그 값은 **어제 정규장**의
변동률이었다. 실제 프리장 등락은 **+0.02%**.

원인: 일봉의 마지막 봉이 정규장 밖에서는 '어제' 다. `closes[-1]` vs `closes[-2]`
로 계산하니 어제의 등락률이 오늘의 실시간 수치처럼 보였다. 숫자 자체는 맞지만
지금 화면을 보는 사람에게는 틀린 정보다.

**설계 결정 두 가지**
1. **평가액·손익의 기준은 계속 정규장 종가.** 확장시간은 거래가 얇아 호가가 크게
   튄다 — 총자산이 새벽에 출렁이면 의사결정에 쓸 수 없는 숫자가 된다.
   확장시간 가격은 '참고 표시' 로만 덧붙인다.
2. **큰 숫자에는 언제 기준인지 항상 붙인다.** 라벨 없는 숫자가 이 오해의 출발점이다.

구현: `us_market_session()` 은 시계만으로 판단해 API 호출이 0이다(정규장 중에는
추가 호출도 없다). 정규장 밖에서만 `_ext_quote()` 가 1분봉+includePrePost 로
마지막 체결을 가져온다. 기준가는 `meta.regularMarketPrice`(직전 정규장 종가) —
⚠ 일봉 `closes[-2]` 를 쓰면 애프터마켓에서 '전전일' 종가가 되어 변동률이 두 배로
부풀려진다.

화면: 종목 상세에 `정규장 종가 기준` 라벨 + `프리마켓 $228.29 +0.14% · 09:18 ET`
한 줄. 보유(시세 모드)·관심 목록에는 작은 칩. 운영 확인 완료.

### 리포트 표현 통일 (오너 지적 ②)

머릿글은 통일했지만 두 리포트가 여전히 따로 놀았다.

| | 이전 |
|---|---|
| 포트폴리오 분석 | 13px 제목 · **강조**와 숫자 색상 · 회색 네모 불릿 |
| 종목 분석 | 11.5px **대문자** 제목 · 강조 없는 평문 · 번호 없음 |

셀사이드 리서치 보고서의 관례를 기준으로 `components/report.jsx` 하나에 모았다.

1. **표지(ReportMeta)** — 목표주가·현재가·상승여력·작성 기준·근거를 본문 위에.
   예전에는 이 정보가 배지와 꼬리표로 흩어져 리포트가 아니라 위젯처럼 보였다.
2. **번호 매긴 섹션** — 01~09. 요약·의견까지 한 배열에 넣어야 번호가 끊기지 않는다.
   제목은 13px/800 하나로 통일. **대문자를 버렸다** — 한글에는 아무 효과가 없고
   라틴 문자만 들쭉날쭉해진다.
3. **강조 렌더러 공유** — `HighlightedText`/`NumHighlight` 를 공용 모듈로 옮겨
   종목 분석도 **굵은 강조와 숫자 색상**을 갖게 됐다. 리서치의 핵심은 숫자다.
4. 섹션 제목 밑 hairline 을 양쪽에 동일 적용(`.mono-section-header`).

**⚠ 이번에 크게 실수할 뻔했다.** 공용 모듈로 옮기려고 파일 범위를 문자열 경계로
잘랐는데 `DaonAIReport`·`DividendSimulator`·`AllocPhase` 까지 **491줄이 지워졌다.**
그런데도 **빌드는 통과했다** — JS 는 미정의 참조를 빌드 타임에 잡지 않는다.
`git diff --stat` 으로 삭제 규모를 보고 알아챘다. 되돌린 뒤 줄 번호 + 함수 개수
검증(`block.count('function ') == 2`)을 걸고 다시 잘랐다(55줄).
**교훈: 리팩터링에서 '빌드 통과' 는 아무것도 보장하지 않는다. diff 크기를 봐라.**

### PWA 설치 안내 (오너 지적 ①)

"휴대폰 설정 알림에 다온이 검색 안 된다" — 매니페스트·아이콘·SW 는 정상이었다.
원인은 **설치 여부**다. 휴대폰 설정의 알림 목록은 *설치된 앱* 만 보여준다.
브라우저 탭으로 쓰는 동안 다온은 앱이 아니라 웹사이트여서, 안드로이드에서는
크롬 안쪽에 파묻히고 아이폰에서는 아예 없다.

설정 탭 알림 카드에 설치 섹션을 넣었다 — 설치 여부를 먼저 보여주고, 안드로이드는
`beforeinstallprompt` 로 **버튼 한 번 설치**(WebAPK), 아이폰은 공유 → 홈 화면에 추가
안내. 설치돼 있으면 "홈 화면 앱으로 실행 중" 으로 확인시켜 준다.
`apple-touch-icon.png` 404 도 함께 고쳤다.

---

## 2026-08-27 — 머릿글 통일 · 투자의견 어휘 · 휴대폰 알림 노출 · 발굴 간격

### 리포트 머릿글을 하나로 (오너 지적 ②)

같은 AI 리포트인데 탭마다 머릿글이 달랐다.

| | 이전 | 이후 |
|---|---|---|
| 분석 탭 | 5px 회색 네모 | 그대로 |
| 종목 분석 (촉매·강세·리스크) | 대시 `–` | 회색 네모 |
| 종목 분석 (요약·개요·실적·컨센서스·의견) | **머릿글 없는 줄글** | 문장마다 회색 네모 |

`components/BulletList.jsx` 하나로 모았다. **대시를 버린 이유**는 금융 화면에서
마이너스 부호와 헷갈리기 때문이다(`– 3.2%` vs `-3.2%`). 원형 점은 design.md 의
직사각형 원칙과 어긋난다. 운영 실측: 네모 마커 46개, 대시 0개.

### 투자의견 '보유' → '중립' (오너 지적 ③)

오너 질문이 정확했다. 보유하지 않은 종목의 분석에 '보유' 가 찍히니
"사서 들고 있으라는 뜻인가?" 로 읽힌다.

증권업계 표준 3분류는 **매수 / 중립 / 매도**(Buy·Hold·Sell)다. 등급을 셋으로 나눈
것 자체는 맞았고, 이름만 틀렸다. '유보' 는 '의견 유보 = 의견을 내지 않음' 이라는
다른 뜻이라 등급명으로 쓰면 "분석을 못 했다" 로 읽힌다.

- 프롬프트 어휘를 '중립' 으로 교체, `utils/reco.js`·`_normalize_reco()` 로 옛 캐시의
  '보유' 도 함께 정규화.
- ⚠ **정규화를 나침반 비교에도 넣어야 한다.** 안 넣으면 재분석 때마다
  보유→중립이 '판단 뒤집힘' 으로 잡혀 가짜 신호가 뜬다.
- 화면에는 뜻을 같이 적는다 — "보유 중이면 유지 · 미보유면 관망".

### 휴대폰 알림 — 있는데 못 찾던 기능 (오너 요청 ①)

Web Push 는 2026-06 에 이미 붙어 있었다(VAPID + service worker + 5분 cron 발송,
±5% 급등락은 2026-07-29 부터). 서버 cron 도 살아 있다(`daon-check-alerts.sh`).
문제는 **켜는 스위치가 알림 벨 시트 안쪽에만 있어서 기능의 존재를 몰랐다**는 것.

`PushSetupCard` 를 설정 탭 상단으로 꺼냈다 — 상태 배지, 무엇이 언제 오는지,
급등락 기준(±3/5/7/10%) 즉시 변경, 테스트 발송. 그리고 **iOS 는 홈 화면에 추가해야
푸시가 온다**는 안내를 조건부로 띄운다(사파리 탭에서는 권한 요청이 조용히 실패한다).
브라우저가 이미 차단한 경우엔 해제 방법을 적는다 — 앱에서는 다시 물어볼 수 없다.

### 발굴 탭 행 간격 (오너 지적 ④)

행 높이가 52px / 35px 로 섞여 있었다. 원인은 AI 의견·임상 배지 줄을 **있는 행에만**
렌더한 것. 모바일에서는 오른쪽 열이 잘려 보여서 간격만 제멋대로인 것처럼 보였다.
배지가 없어도 자리를 차지하도록 `minHeight` 를 줬다. 운영 실측 16행 전부 52px.

### cron 엔드포인트 노출 차단 (오너 대기 항목 정리 중 발견)

`/api/cron/warm_prices` 는 인증이 없고 한 번 호출에 **13.6초**가 걸렸다.
개인정보는 나가지 않지만, 아무나 반복 호출하면 1GB 서버의 CPU·메모리를 그대로
먹는다. Cloudflare 규칙으로 막는 것을 오너 작업으로 미뤄뒀었는데, 코드에서
막는 편이 확실하다 — 방화벽 설정은 잊히고, 코드는 남는다.

`_require_local_caller()` 로 서버 자신의 호출만 통과시킨다.
⚠ 리버스 프록시가 앞에 있으면 `client.host` 가 전부 127.0.0.1 로 보이므로,
`x-forwarded-for`·`cf-connecting-ip`·`x-real-ip` 중 하나라도 있으면 외부로 판정한다.
서버 안의 curl 은 이 헤더를 붙이지 않는다.

검증: 서버 내부 200(15.6초, cron 정상) · 외부 403. cron 스크립트는 손대지 않았다.

### 암호화폐 (오너 질문 ⑥)

**이미 지원되고 있었다.** `BTC` 입력 → 저장 시 `BTC-USD` 정규화, 한글명 자동,
시세는 미국 주식과 같은 경로(5분 캐시 워밍). 등록 탭 placeholder 를
`AAPL · 005930 · BTC` 로 바꿔 세 형태를 한눈에 보이게 했다.

---

## 🔴 2026-08-27 — 자산 노출 사고 원인 확정 + 가림 모드 전면 적용

### 원인이 확정됐다 — 이전 추정은 반쪽이었다

오너 확인: **자기 PC에서 앱을 띄워 팀에 기능 시연을 하던 중** 노출됐다.

먼저 "우연의 일치" 가설부터 지웠다. 데모 계정 자체 총자산은 **₩137,354,902(1.3억)**
로, 보고된 6억과 겹치지 않는다.

재현해 보니 원인은 단순했다. **가림(privacy) 모드가 포트폴리오 탭에만 걸려 있었다.**
가림을 켠 채 탭을 돌면:
...
| 포트폴리오 | 0 (정상) |
| **분석** | ₩ 금액 8곳 · 억 단위 7곳 |
| **설정** | ₩ 금액 5곳 (총 투자금 · 계좌별 예수금) |

AI 전략 리포트 본문("총자산 ₩618,010,693 …")은 우리가 포맷하지 않은 문장이라
가림 로직을 아예 통과하지 않았다. 이것이 "포트폴리오 안내 글에 6억이 노출된"
바로 그 경로다.

**가장 위험한 부분은 화면에 '가림'이라고 적혀 있었다는 것이다.** 안전하다고
믿으니 발표를 계속하게 된다.

### 조치 — 금액을 찍는 자리를 하나로 모았다

`frontend/src/utils/privacy.js` 신설. `usePrivacy()` · `maskText()` 를 거치지 않으면
가림이 적용되지 않는다는 규칙을 만들고, 다음을 전부 통과시켰다:

AllocationTab(도넛 총합·범례·수량·배당 시뮬) · NetWorthChart(축·툴팁·최고/최저) ·
GoalsCard(목표 금액·궤적 축) · BacktestSection(축) · AccountCashCard(예수금 —
가림 중에는 입력칸을 읽기 전용 마스크로 대체) · ManageTab(총 투자금) ·
TransactionsSection(거래 금액) · AI 본문 3경로(NumHighlight · HighlightedText 의
**강조** 구간 · breakSentences).

**AI 본문 경로를 세 번에 걸쳐 찾았다.** 처음엔 NumHighlight 만 막았는데
① `**강조**` 구간은 NumHighlight 를 안 거치고 ② `breakSentences` 라는 별도
경로가 또 있었다. 운영 스캔이 매번 잡아줘서 세 번째에 0이 됐다 —
**정적 검토만으로는 못 찾았을 경로다.**

문장 속 금액 패턴도 보강했다. AI 는 `6천만원` `27만원` 처럼 쓴다 —
조/억/천만/백만/만 을 한 번에 잡고, 쉼표만 찍힌 7자리 이상 숫자도 가린다.
퍼센트는 남긴다(규모를 드러내지 않으면서 무슨 화면인지는 알 수 있어야 발표가 된다).

### 재발 감지

`node scripts/privacy-scan.mjs [URL]` — 가림을 켠 채 전 탭을 돌며 금액 패턴을
센다. 하나라도 남으면 실패한다. CLAUDE.md 필수 확인 4번과 핸드오프 '하지 말 것'에
규칙으로 넣었다.

**검증**: 운영 9개 탭 전부 0건. 가림을 끄면 정상 표시(포트폴리오 28 · 분석 22 ·
설정 5) — 과잉 마스킹 없음.

---

## 2026-08-26 — 테마 정리 (auto 노출 + '프로' → '차콜')

### 먼저, 잘못된 진단을 정정

"다크와 프로가 구분이 안 된다"고 봤는데 **틀렸다.** 토큰 40개를 Lab 색공간에서
재보니 배경·카드 ΔE 7.0/7.5(나란히 놓으면 구분됨), 하락·정보·AI 색 ΔE 19.8/23.3/23.9
(뚜렷이 다름)이었다. 다크는 네이비 계열, 프로는 중성 회색 계열이다.

### 진짜 문제 — 순환 순서와 설정 카드가 어긋나 있었다

`cycleTheme` 순서는 `light → dark → pro → auto` 4개인데 설정 탭 카드는 3개였다.
그래서 **auto 는 상단 알약을 4번 눌러야만 걸리는 숨은 값**이었고, 라벨 판정이
"light 도 dark 도 아니면 프로"라서 **auto 일 때 '📈 프로' 로 잘못 찍혔다.**
OS가 라이트면 화면은 하얗게 바뀌는데 라벨은 프로 — 뭘 누른 건지 알 수 없었다.
앱 모드 알약(App.jsx)과 웹 모드 알약(TopNavBar.jsx) 둘 다 같은 버그였다.

### 조치

1. **정본을 `store.js` 로 통합** — `THEME_ORDER` · `THEME_META`(아이콘·라벨·설명·
   미리보기색). 순환 순서 바로 옆에 둬서 다시 어긋나지 않게 했다.
   알약 2곳과 설정 탭이 모두 이 표만 본다.
2. **설정 탭에 '자동' 카드 노출** — 3개 → 4개. 좁은 폭에서 접히도록
   `repeat(auto-fit, minmax(96px, 1fr))`.
3. **'프로' → '차콜'** — 이름이 '프로'라 정보가 더 조밀해질 거라는 기대를 주는데
   실제로 바뀌는 건 색뿐이었다. 색으로 부르고 설명도 색 기준으로 바꿨다
   (`네이비 계열 다크 · 초록 강조` / `중성 회색 다크 · 파랑 강조`).
   ⚠ 저장 key 는 `'pro'` 그대로 — 바꾸면 기존 사용자 설정이 날아간다.

**검증**: OS 라이트/다크 각각 4회 순환 — stored/applied/pill 전부 일치.
설정 탭 카드 4개가 360·390·1440 에서 모두 한 줄, 가로 넘침 없음, pageerror 0.

---

## 2026-08-26 — 터치 타깃 확대 + 분석 탭 목차

### ① 터치 타깃 — 손가락 기준으로 재조정

390px 실측에서 44px 미만 타깃이 여럿 있었다. 최악은 **보유 행의 액션 버튼 26×26**
(수정·노트)이 4px 간격으로 세로로 붙어 있던 것 — 13개 종목 × 2 = 26개.
상단 알약(테마·웹 전환)은 23px, 마켓바 이동 버튼은 폭 26px 이었다.

**원칙**: 시각 크기는 되도록 유지하고 **히트 영역만** 넓힌다. 전부 44px 로 키우면
한 화면 정보량이 크게 준다. `.tap-target`(양방향)·`.tap-target-y`(세로만) 유틸리티를
`::after` 로 구현했다.

⚠ **히트 영역을 넓힐 때는 이웃이 누를 수 있는 요소인지 먼저 본다.** 이웃 버튼을
덮으면 그 버튼이 안 눌린다. 그래서 가로로 붙은 세그먼트는 세로로만 넓혔고,
행 액션 버튼은 **중심 간격(30+10=40) = 히트 크기(40)** 로 맞춰 서로 맞닿되
겹치지 않게 했다.

| 대상 | 전 | 후(실효) |
|---|---|---|
| 보유 행 수정·노트 | 26×26 | 30×30 시각 / **40×40** 히트 |
| 알림 벨 | 32×32 | 32×32 시각 / **44×44** 히트 |
| 테마·웹 전환 알약 | 23px | **40px** |
| 마켓바 ‹ › | 폭 26px | 폭 **40px** |
| 세그먼트(.seg-btn) | 27px | **36px** |
| 계좌 셀렉트 | 30px | **40px** |
| 가림/표시·계산·저장·MD | 27~33px | **44px** 히트 |
| 종목 탭 보유 칩 | 27px | **36px** |

데스크톱(마우스)은 `--tap-*` 가 전부 0 이라 **기존 밀도 그대로**다(실측: seg-btn 27px,
액션 버튼 26×26, 행 높이 81px 로 변화 없음). 앱 모드에서만 켜진다.

**비용**: 보유 행 높이 79 → 93px(+18%). 13종목 기준 목록이 182px 길어졌다.
되돌리거나 조절하려면 `App.css` 의 `--tap-act`(버튼 크기)·`--tap-act-gap`(간격)
두 값만 고치면 된다.

### ② 분석 탭 목차 (ChapterNav)

분석 탭은 3~5화면이라 원하는 챕터까지 스크롤로만 가기 어려웠다. 스크롤 컨테이너
안에 sticky 로 붙는 목차 바를 넣었다 — 탭하면 그 챕터로, 스크롤하면 현재 챕터가
표시된다.

- **챕터 목록을 하드코딩하지 않는다.** `[data-chapter]` 를 DOM에서 읽는다.
  Ⅱ장·부록은 보유가 없으면 렌더되지 않아, 하드코딩하면 죽은 버튼이 생긴다.
- **함정 1 — 첫 렌더에서 `items.length < 2` 로 null 을 반환하면 `wrapRef` 가 DOM에
  안 붙어 스크롤 컨테이너를 못 찾는다.** 그래서 `items.length` 를 의존성에 둬
  목차가 실제로 그려진 뒤 리스너를 다시 붙인다.
- **함정 2 — 스크롤포트의 윗변은 `getBoundingClientRect().top + padding-top`.**
  웹 모드 `.web-main-col` 은 padding-top 24px 이라, 이걸 빼먹으면 점프 후 챕터
  제목이 딱 24px 만큼 목차 바 뒤로 숨는다.
- `scrollIntoView` 대신 직접 계산해 옮기고, 550ms 뒤 최대 2회 보정한다
  (스크롤 중에 위쪽 지연 로딩 카드가 붙으면 목표 지점이 밀린다).

**검증**(Playwright, 앱 390 / 웹 1440):
챕터 4개 인식 · 점프 후 제목이 목차 바에 가리지 않음 · 스크롤 시 현재 챕터 갱신 ·
console/pageerror 0. 히트 영역 가장자리(시각 버튼 밖 4px) 탭이 해당 버튼으로 잡히고,
노트 시트 열림/닫힘·행 탭(차트 이동)·가림 토글 모두 정상. pytest 209 통과.
