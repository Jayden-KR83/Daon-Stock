...
> 마지막 업데이트: **2026-08-26** (개인 자산 노출 사고 대응)
> 모델: claude-opus-4-8
...
## 🔴 2026-08-26 — 개인 자산 노출 사고 대응

**보고**: 데모를 쓰던 동료 화면에 오너 자산 규모(6억대)가 보였다.

### 조사에서 나온 것

**결함 1 — 공유 캐시에 개인 재무 리포트.** `ai_cache` 는 전 사용자가 공유하는
캐시다(원래 용도: 종목 분석 `stock_v2`). 그런데 AI 전략 리포트 `strategy:<uid>:<fp>` 가
**7행** 쌓여 있었고, 그 안에 총자산 ₩618,010,693·계좌별 평가액·환율 노출 비중이
평문으로 있었다. 오너 실제 총자산(₩614,800,732)과 보고된 "6억"이 일치한다.

조회 함수가 정확한 키를 요구해 *당장* 교차 조회는 되지 않았다. 문제는 같은 파일에
이미 `stock_v2:{TICKER}:%` 로 **접두 검색**하는 함수가 있다는 것이다. 접두 검색을
하나만 더 잘못 추가하면 즉시 유출된다 — **사고 대기 상태**였다.

**결함 2 — 브라우저에 남던 개인값.** `daon_retire_years`·`daon_monthly_inflow`·
`recentTickers` 가 사용자 구분 없이 `localStorage` 에 저장되고 로그아웃해도 지워지지
않았다. 같은 기기를 쓰면 다음 사람(데모 포함)의 화면이 앞사람의 은퇴 계획·월납입액·
조회 종목을 그대로 쓴다.

### 조치
1. 운영 DB 에서 개인 캐시 7행 삭제 + `VACUUM`(파일에 남은 평문 회수). 백업 선행.
2. `_PRIVATE_CACHE_PREFIXES` 도입 — 개인 스코프 키는 공유 캐시에 **저장도 조회도** 안 된다.
   전략 폴링은 `strategy_cache`(user_id+scope+fingerprint) 폴백이 있어 동작 불변.
3. 접두 검색 함수에 `stock_v2` 전용 단언 추가.
4. `setAuth` 에서 토큰이 **실제로 바뀔 때만** 개인 로컬값 삭제
   (authMe 응답마다 같은 토큰으로 재호출되므로 매번 지우면 사용 중에 값이 날아간다).

### 재발 방지
- **CLAUDE.md '작업 시작 전 필수 확인' 4번으로 격상** — design.md R1~R6 과 같은 강제 규칙.
- `tests/test_shared_cache_privacy.py` 9건 — 개인 키 저장·조회 차단, 종목 분석 정상 동작,
  **공유 캐시에 `stock_v2` 이외 키가 남아 있으면 실패**(잔존물 조기 발견).
- `PERSONAL_LOCAL_KEYS` 주석에 "새 개인값 추가 시 반드시 등록" 명시.

### 정직한 한계
동료가 실제로 본 화면의 렌더 경로는 **재현하지 못했다.** 조치 전에도 데모 세션의
API 응답 37건과 DOM 전수 스캔에서 오너 자산 지문이 나오지 않았다. 위 두 결함은
보고와 부합하는 실제 결함이지만 "이것이 그 경로였다"고 단정할 수는 없다.
같은 기기 공유 여부가 확인되면 결함 2로 확정된다.

### 검증
운영 삭제 후 잔존 0건 · 조치 후 데모 재스캔 0건 · 앞사람 개인값 3종이 데모 진입 시
전부 삭제되는 것 실측 · 사용자별 테이블 격리 전수 점검(비격리 1건은 가격 워밍 cron 으로
사용자 데이터 미반출) · pytest 209 통과.

---

## 🆕 2026-08-24 세션 — 분석 탭 재편 (카드 정리 · 순서 · 계좌 단위 진단)

**한 줄 요약**: 분석 탭이 너무 길고 흐름이 끊겼다. 판정 기준을 세워 카드를 솎고,
"지금 어떤가 → 뭐가 문제인가 → 그래서 뭘 할까" 한 방향으로 다시 배열했다.

### 판정 기준 (오너 승인)
(A) 이걸 보고 실제로 행동이 바뀌는가 (B) 다른 카드와 중복인가
(C) 실측인가 추정·AI 생성인가 (D) 화면 길이·API 비용

### 삭제 2건 — 근거를 남겨둔다
- **백테스트**: 지금 보유한 종목은 **이미 살아남은 종목**이라 과거 성과가 구조적으로
  과대평가된다(생존 편향). 게다가 '내 실제 성과'는 Ⅰ장 자산 추이가 이미 보여준다.
- **글로벌 매크로 포지셔닝**: 예측이고, 개인 포트폴리오의 행동을 바꾸지 않으며,
  틀렸을 때 피해가 크다(A 낮음·C 낮음).

### 분리 1건
- **저점발굴 시계열**: 발굴 탭이 따로 있어 중복. 투기 영역을 전략 리포트에 섞으면
  '핵심 자산'처럼 읽힐 위험이 있다.

### 통폐합 4건
- 전문가 총평 + 리스크 진단 + 주요 위험 요소 → **한 카드**(내용은 유지, 카드만 통합)
- 건강도 100점 → **경고 카드 위 한 줄 배지**. "72점"이라는 숫자는 다음 행동을 지시하지
  못한다. 행동은 아래 경고에서 나온다. 하위 지표 해설은 '자세히'로 접어 두었다.
- 요약 배너의 **총 평가액 제거** — 포트폴리오 탭 상단이 같은 값을 이미 크게 보여준다.
  같은 숫자를 두 탭에서 반복하면 정보가 아니라 소음이다. 손익·구성만 남겼다.
- MD 내보내기 버튼 → 최상단에서 **부록(맨 아래)** 으로. 사용 빈도가 낮다.

### 순서 — 배당이 두 번 나오던 문제
예전에는 배당 **이력**이 Ⅰ장, 배당 **시뮬**이 Ⅲ장이라 "지금 얼마 받나"와
"목표를 넣으면 이렇게"가 서로 이어지지 않았다. 배당을 Ⅲ장으로 모아 한 흐름으로 붙였다.

Ⅰ 지금 내 상태(자산 추이·비중) → Ⅱ 무엇이 문제인가(경고·건강도·AI 진단)
→ Ⅲ 그래서 무엇을 할까(목표 → 자산배분 → **배당: 현재→목표→방법** → 추천 액션) → 부록

구현: `DaonAIReport` 에 `part` prop 을 넣어 한 리포트를 챕터별로 나눠 그린다
('diagnosis' | 'alloc' | 'dividend' | 'actions'). 상태는 하나, 그림만 여러 곳이다.

### 계좌 단위 진단 (5번)
경고·건강도에 계좌 필터를 붙였다. **서버 변경 0** — 백엔드가 보유 목록을 요청 본문으로
받는 구조라 프론트에서 걸러 보내면 그만이었다.

⚠️ 기본값은 **전체**로 두었다. 계좌 하나만 보면 분산이 실제보다 나빠 보이기 때문이다
(ISA에 반도체만·퇴직에 채권만이면 각각은 쏠림이지만 합치면 균형). 계좌를 고르면
그 취지의 주석이 함께 뜬다. 실측 확인: 퇴직 계좌 기준으로 SK하이닉스가 26% → **52.8%**
로 다시 계산됐다 — 계좌 단위가 왜 과장되는지를 그대로 보여주는 사례다.

### 곁가지 — 투자 나침반이 안 보이던 이유
배너는 정상이었다. **신호 조회 창이 3일**인데 신호 2건이 모두 5.2일 전이라 창 밖이었다.
주말·연휴를 끼면 통째로 놓친다 → **7일로 넓혔다.**
또 두 신호 모두 `prev_reco` 가 비어 있는 **첫 분석**인데 "분석 없음 → 보유로 바뀌었습니다"
라고 표시하고 있었다. 사실과 어긋나므로 첫 분석과 판단 변경을 문구로 구분했다.

### 검증
390px·1280px 양쪽에서 섹션 순서를 실제 렌더로 측정해 의도한 9개 순서와 일치 확인.
삭제 4종(백테스트·매크로·저점발굴·총평 카드) 화면 부재 확인, 계좌 필터 클릭 후
주석·재계산 확인. 콘솔 에러 0, 가로 오버플로 0. pytest 200 통과.
AI 리포트 파트 배치는 **합성 리포트를 로컬 캐시에 주입**해 API 비용 0으로 확인했다.

---

## 🆕 2026-08-21 세션 (4차) — AI 전략 리포트 어조를 합니다체로 고정

**증상**: 분석 탭 '추천 액션'·리밸런싱 문장만 해라체로 나와 같은 탭 안에서 어조가 튀었다.
정적 문구는 전부 존댓말이었고(스캔 결과 0건), **전부 AI 산출물**이었다.

**원인**: 프롬프트에 어조 규칙이 아예 없었다. 게다가 작성 지침 자체가 명령조
("동결하라", "다루지 마라")여서 그 문체가 산출물로 새어나갔다.

**조치**: `[어조]` 블록을 추가하고 금지 예시를 명시했다. 핵심은 마지막 한 줄 —
"지침문이 명령조여도 산출물의 어조는 이 규칙을 따릅니다". 모델은 예시 문체를
그대로 따라가므로 `actions`·`rebalancing` 필드 설명에도 합니다체 예문을 넣었다.

**캐시 처리**: `STRATEGY_PROMPT_VERSION` 을 지문에 섞었다. 없으면 보유 종목이
그대로인 한 예전 어조의 캐시가 계속 나온다. 캐시 **조회**는 (user, scope) 기준이라
화면에서 리포트가 사라지지는 않고, 다음 '분석' 실행 때 새 어조로 재생성된다
— 자동 재생성이 아니므로 추가 과금은 없다.

**검증**: 같은 지침을 **구독 경로(`claude -p`, 0원)** 로 실행해 산출물이 전부
합니다체("권고합니다"·"바랍니다")로 나오는 것을 확인했다. 종량제 API 를 쓰지 않고
어조 규칙만 따로 시험할 수 있다는 점이 유용했다. pytest 200 통과.

---

## 🆕 2026-08-21 세션 (3차) — 모바일 보유 목록 종목명 잘림

**증상**: 앱(모바일) 폭에서 `TIGER 미국…` 처럼 이름이 잘려, 정작 그 종목을 알아볼
단서가 사라졌다. 원인은 좁은 칸에 **이름과 티커를 함께** 넣으려 한 것.

**판단 — 모바일에서는 하나만 보여준다.** 사용자가 그 종목을 부르는 말은 하나다.
한국 종목은 한글 종목명, 미국 종목은 티커(AAPL·SPY). 보조 식별자는 좁은 화면에서
차지하는 자리에 비해 값이 낮다. 데스크톱은 폭이 넉넉하므로 **기존 그대로 두었다**
(`.app-mode-app` 스코프로만 규칙 적용 — 뷰포트 폭이 아니라 실제 레이아웃 모드 기준).

**2줄에서 끊는 이유**: TDF2045 류 펀드명은 3줄을 줘도 다 안 들어가는데 행 높이만
들쭉날쭉해져 목록 훑어보기가 무너진다. 2줄로 고정하고 나머지는 말줄임,
전체 이름은 행 `title`(길게 누르기)과 종목 상세에서 본다.

**폭도 같이 넓혔다**: 보조 식별자를 감춘 것만으로는 한 줄에 9~10자였다.
스파크라인 칸을 조금 양보받아(1.3fr→1.75fr) `TIGER / 미국나스닥100T…` 까지 늘렸다.
한글은 `word-break: keep-all` 로 단어 단위로 끊고, 공백 없는 긴 영문·숫자만
`overflow-wrap: anywhere` 로 강제로 접는다.

**검증**: 22자·35자 펀드명을 일부러 넣은 계정으로 360/390/430px 실제 렌더 측정
(줄 수·잘림 여부·보조 식별자 표시 여부). 가로 오버플로 0, 콘솔 에러 0.
1280px 에서 보조 식별자 표시 유지 확인 = 데스크톱 무변경.
