...
## 2026-09-16 세션 — ⚠️ 커밋만 됐고 운영 미배포

레이더 2주차 실행 + 결함 4건 수정(배당 '예정'에 지난 날짜·이중 합산 / 의견 버튼이 마지막 행
덮음 / 관심 삭제 오터치 / 캘린더 ‹ › 폭 18px). 백엔드 테스트 216 통과(신규 7).
**배포 명령이 자동 모드 권한에 막혀 운영엔 안 올라갔다.** 빌드 산출물은 로컬에 있다.
다음 세션 첫 일: 복구 순서 4번(번들 해시)을 **서버 쪽과** 비교 → 다르면 `docs/deployment.md` §2
대로 assets → index.html·sw.js·workbox → main.py 재시작 → `guard:all`.

## 남은 것 (우선순위)

0. **09-16 변경분 운영 배포** — 위 블록.
