...
> **📍 2026-08-24 현재 상태는 바로 아래 "현재 운영 구조" 한 블록만 읽으면 된다.**
...
# 📍 현재 운영 구조 (2026-08-24 기준) — 새 세션은 여기까지만 읽으면 된다
...
- **분석 탭 재편(2026-08-24) 완료** — 카드 삭제 2(백테스트·매크로)·분리 1(저점발굴)·
  통폐합 4(총평+위험요소→진단 한 카드 / 건강도→한 줄 배지 / 요약배너 총액 제거 /
  MD버튼→부록). 순서는 **Ⅰ 지금 내 상태 → Ⅱ 무엇이 문제인가 → Ⅲ 그래서 무엇을 할까**.
  배당은 이력+시뮬을 Ⅲ장 한 곳으로 모았다. 구현은 `DaonAIReport` 의 `part` prop
- **계좌 단위 진단(2026-08-24) 완료** — 경고·건강도에 계좌 필터(서버 변경 0).
  **기본값 전체 유지** — 계좌 하나만 보면 분산이 실제보다 나빠 보인다(주석으로 안내)
- **투자 나침반 신호 창 3일→7일(2026-08-24)** — 신호가 창 밖으로 밀려 배너가 안 뜨던 원인.
  `prev_reco` 가 빈 '첫 분석'은 '판단 변경'과 문구를 구분한다
- **모바일 보유 목록 라벨(2026-08-21) 완료** — 앱 모드에서 한국=한글명 / 미국=티커
  **하나만** 표시하고 최대 2줄 줄바꿈. 데스크톱은 무변경(규칙이 `.app-mode-app` 스코프).
  긴 펀드명은 2줄에서 말줄임 — 전체 이름은 행 title·종목 상세에서 확인
- **계좌별 예수금(2026-08-21) 완료** — `accounts.cash`(계좌 통화 기준 저장) +
  `PUT /api/accounts/{key}/cash`. 설정 탭 '계좌별 예수금' 카드에서 입력.
  총자산(포트폴리오 상단)·현금 비중(분석 탭)에 반영되며 **손익·수익률은 주식만 기준 유지**.
  뷰별 처리: 계좌별=그 계좌에 합산 / 섹터별·종목별='현금' 한 칸(상위 15 컷 뒤에 붙임)
- **최초 로그인 온보딩 투어(2026-08-21) 완료** — 10단계 코치마크. 사용자별 1회 자동,
  설정 탭 → '안내 다시 보기'로 재실행. 보유 0개면 '첫 종목 추가' 안내로 자동 전환.
  Playwright 로 360·390·1280px 실제 클릭 완주 검증. 구현 `components/Tour.jsx`
  · 대상 앵커는 각 컴포넌트의 `data-tour="..."` 속성
