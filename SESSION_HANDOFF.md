...
**상태**: 로컬 커밋 완료 + `origin/main` 푸시 완료. **미배포**(서버 반영 안 됨).
pytest **160 통과** · vite build OK.
...
### ④ 투자 나침반 — **설계안까지만. 오너 승인 대기** → `docs/investment-compass-design.md`
핸드오프가 "구현 전 비교표 + 승인"으로 정의한 작업이라 코드 미작성.
...
- **권고: A(보유연동 한 줄, 추가비용 ~0) → B(매크로 탭, 월 $3~5) 순차. C(유튜브 원안) 비권장**
  — 비용 최고·신뢰도 최저·기술리스크 유일

---

## 다음 후보 (우선순위 순)

1. **배포** — 이번 5건이 전부 미배포다. `docs/deployment.md` §5 체크리스트
2. **④ 승인** — A/B/C 중 선택 (문서 §3 승인 필요 사항 3가지)
3. **② cron 등록** — 서버 SSH 작업
