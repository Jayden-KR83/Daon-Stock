...
> **📍 2026-09-23 현재 상태는 바로 아래 "현재 운영 구조" 한 블록만 읽으면 된다.**
...
# 📍 현재 운영 구조 (2026-09-23 기준) — 새 세션은 여기까지만 읽으면 된다

> **이전 블록(2026-08-24)의 "분석 갱신은 서버 cron 이 아니다"는 더 이상 사실이 아니다.**
> 오너 요청으로 주간(08-29)·일일(09-14) 서버 cron 이 되살아났다. 아래가 정본이다.

## 갱신 구조 — 3중

| 주기 | 무엇 | 경로 | 비용 |
|---|---|---|---|
| **매일 07:30 KST** | 전략 리포트(분석 탭) | 서버 `daon-daily-strategy.sh` · `max_users=20` | 1명 월 ≈ $1.8 |
| **매주 월 06:00 KST** | 전략 리포트 + 종목 분석 25종목 | 서버 `daon-weekly-refresh.sh` | 월 ≈ $8 |
| 매일 06:00 | 보유 종목 AI 분석 | 오너 PC 작업스케줄러 + 구독 | 0원 |
| 매일 07:00 KST | 발굴 스캔 | 서버 `daon-discover-scan.sh` | AI 미사용 |

**왜 전략만 매일이고 종목 분석은 주 1회인가** — 취향이 아니라 돈이다.
전략 리포트 ≈ $0.06/회, 종목 분석 ≈ $0.08 × 25종목(매일이면 월 $60).
기업 펀더멘털은 하루 만에 바뀌지 않고, 시세·비중은 매일 바뀐다.

**AI 가 멈추면 먼저 볼 것 — 크레딧.** 다온은 **API 키 지갑**(console.anthropic.com)을 쓴다.
claude.ai 구독의 "사용 크레딧"과는 **다른 지갑**이다. 2026-09-11 오너가 구독 쪽에 $250 을
넣었는데 API 는 -$0.17 이라 AI 가 며칠 멈췄다.
확인: 서버에서 `settings.anthropic_key` 로 haiku 1회 호출 → `credit balance` 문구면 충전 문제.
**자동 충전은 아직 꺼져 있다** — 오너가 켜야 한다.

## 🔴 지금 지켜야 하는 규약 (CLAUDE.md 에 정본)

1. **숫자 규약(2026-09-14)** — 같은 이름은 같은 것을 가리킨다. '자산'은 주식+예수금만.
   주식만이면 '주식 평가액'. `npm run guard:numbers` 로 탭 간 일치를 본다.
2. **개인 데이터 격리** — 공유 캐시엔 `stock_v2:` 만. 금액은 `usePrivacy()`.
3. **실명을 화면에 띄우지 않는다(2026-09-10)** — `utils/userName.js` 의 `displayNameOf` 만.
   서버 세션도 `nickname` 자리에 `name` 을 대신 채우지 않는다. DB 의 실명은 전부 삭제됐고
   복구용 스냅샷은 `settings.purged_names_2026-09-10` 에 있다.
4. **주 1회 자가발전 루프(daon-radar)** — `.claude/skills/daon-radar/SKILL.md`,
   기록 `docs/RADAR.md`. 마지막 실행 2026-09-16(2주차). 다음 회차는 주 단위로 `guard:radar` 가 감시한다.

## 가드 — 변경 후 반드시

```
cd scripts && npm run guard:all
```

답변분해 · 레이더 정지감지 · 개인정보 9탭 · **탭 간 숫자 일치** · 첫사용 UX(자기검사 포함).
⚠ 에셋 업로드 **도중에** 돌리면 옛/새 번들이 섞여 숫자 검사가 거짓 실패한다. 배포가 끝난 뒤 돌린다.

## 완료 — 다시 확인할 필요 없음 (2026-09-09 ~ 09-15)

- **지인 첫사용 피드백 10건** — 티커↔종목명 양방향 자동완성(한글 미국주는 별칭표),
  평단 콤마, 예시값 반복 제거, 행 추가 고정·스크롤, Market Status 위치, 이름 잘림,
  포트폴리오에서 **팝업 등록**(`QuickAddModal`, `addHolding` 한 건만 — 전체교체 금지),
  웹·앱 양쪽 가림 토글, 종목 차트 평단선+손익
- **탭별 총자산 불일치 해소** — 스냅샷이 예수금을 저장한 적이 없었다. 카드명을
  '주식 평가액 추이'로 바꾸고 "+ 예수금 = 총자산" 병기. `net_worth_snapshots.cash_krw`
  를 오늘부터 누적한다(NULL=몰랐다 / 0=없었다)
- **PWA 설치 경로 복구** — `beforeinstallprompt` 를 index.html 에서 선점. 삼성 인터넷 안내
- **채팅 답변 갈무리** — 노트 저장(`/api/notes/{t}/append`, 상한 12000자 단일화) · 집현전용 복사
- **인앱 피드백** — `의견` 버튼 · `/api/feedback` · 화면·기기 자동 첨부 · 금액 미포함
- **동시접속 1차 병목** — 스레드풀 40→128. **workers 증설은 금지**(하지 말 것 참고)
- **AI 오류 한국어화** — 400 원문(영문 JSON)을 화면에 흘리지 않고 서버 로그로만
- **발굴 탭** — 모바일 열 접기, 필터 고정(sticky), `종류` 라벨, 토글 17px(터치 37px 유지),
  섹터 바깥 클릭 닫기
- **전략 분석 이어달리기** — 폴링을 `utils/strategyJob.js` 로 빼 탭을 옮겨도 계속 돈다
- **단축키 = 좌측 메뉴 순서** — `SideNavBar` 의 `SHORTCUT_TABS` 가 정본
- **하단바 끌기 전환** · 화이트 테마 명도차 완화 · 신규 사용자 기본 테마 auto
- **`/api/cron/warm_prices` 외부 차단** — 2026-09-16 실측 외부 403(예전 200)

## 2026-09-23 세션 — 배포 완료 + "2분째 안 뜬다"의 진짜 원인

**느림의 원인은 앱이 아니라 MTU 였다.** 같은 요청이 서버 내부 0.003s / 오리진 직접 0.03s /
Cloudflare 경유 0.2~60s. 인터페이스 MTU 9000 인데 경로 MTU 는 1500 이라 큰 패킷이
조용히 버려지고 TCP 가 재전송 백오프로 수십 초를 먹었다. 조사·조치 전문은
[docs/deployment.md](docs/deployment.md) §8.

- `ens3` MTU 1500 고정(`daon-mtu.service`) · `tcp_mtu_probing=1` · `vm.swappiness=10`
- 09-16 미배포분 + 캐시 상한을 **배포 완료**(`main.py` sha 일치 · 번들 `index-B4LeyTaf.js` ·
  `guard:all` 5종 통과 · 배당 '예정' 지난 날짜 0건 실측 · 하단 겹침 0 실측)
- **ttl_cache 에 상한(1200)·만료 청소 추가** — TTL 이 항목을 지우지 않아 8일에 1.4GB 까지
  불었다(RSS 445MB + 스왑 985MB). `_ai_cache`(500) `_earnings_cache`/`_dividends_cache`(1000)
  `_kr_price_*`(4000)도 같이 묶었다. 테스트 `tests/test_cache_bounds.py` 6건.

⚠ **남은 것: CF↔오리진 잔여 패킷 손실.** MTU 수정 후에도 연결에 따라 재전송 14~44% 가 남아
12회 중 1~2회 2~10초 스파이크가 난다. 서버가 **밖으로 여는** 연결은 손실 0 이므로 해법은
Cloudflare Tunnel(cloudflared)로 인바운드 경로를 없애는 것. 오너 결정 대기.

## 남은 것 (우선순위)

0. **Cloudflare Tunnel 검토** — 위 잔여 손실. 이것만 남기면 스파이크가 사라진다.
1. **API 자동 충전 켜기** — 오너 작업. 꺼져 있어 잔액 0이면 AI 가 조용히 멈춘다.
2. ~~daon-radar 정규 회차~~ — 09-16 실행 완료. 다음 회차 09-21(월) 전후. 로드맵 '검토 예정'에
   실적발표 전날 푸시 · 급등락 푸시 관련 뉴스 1줄 추가(착수는 오너 결정).
3. **백업 개인키 오프라인 사본** — 오너 작업. 오너 PC 홈 아래 `.daon-backup-key` 폴더의 두 파일.
4. **로드맵 '검토 예정'(착수는 오너 결정)** — 자산군 나침반(금·은·환율 백분위) ·
   전문가 모드=정보밀도 · ETF 관통 노출 · 복권형 편향 진단 · 드로다운 이력 · Ampere A1 이전
5. Dependabot ON · PBKDF2 상향 · 세션 만료 단축(`docs/security-review.md` §4)
6. 백업 오프사이트 복사 — 현재 서버 한 대에만 있다

**오너 결정으로 확정된 것**: 가입 **수동 승인제 유지**(09-11) · 앱 내 애드센스 기각 ·
구독 사용 크레딧 $250 은 환불 불가(1년 유효, Claude Code 용)

## 🚫 하지 말 것
- **공유 캐시(`ai_cache`)에 개인 스코프 결과를 넣지 말 것.** 2026-08-26 자산 노출 사고의
  원인. 전략 리포트는 `strategy_cache`(user_id+scope) 가 정본이다. 코드로 차단돼 있으니
  우회하지 말 것 — 우회하면 `tests/test_shared_cache_privacy.py` 가 실패한다.
- **AI 답변을 화면용으로 쪼갤 때 `**강조**` 짝을 깨지 말 것.** 별표가 그대로
  노출된다(2026-09-03 사고). `utils/answerText.js` 하나만 쓰고 CI 검사를 믿을 것.
- **길이 상한에 걸려 끊긴 답을 완성된 답처럼 보여주지 말 것.** `stop_reason` 을 본다.
- **공용 계정(데모)에 사용자 입력을 저장하지 말 것.** 채팅 대화를 저장했더니 다음
  방문자에게 앞사람 질문이 보일 뻔했다(2026-08-28). 데모는 여러 사람이 같이 쓴다.
- **금액을 `₩${v.toLocaleString()}` 처럼 직접 찍지 말 것.** `usePrivacy()` 를 쓴다.
  가림 모드가 그 화면에서만 안 걸리는데, 화면에는 '가림'이라 적혀 있어서
  안전하다고 믿게 된다 — 2026-08-27 확정된 자산 노출 경로다.
- **개인 값을 localStorage 에 넣고 `PERSONAL_LOCAL_KEYS` 등록을 빠뜨리지 말 것.**
  로그아웃해도 안 지워져 같은 기기의 다음 사용자(데모 포함)에게 그대로 넘어간다.
...
- ~~서버 cron 으로 분석 갱신 되살리지 말 것~~ — **폐기(2026-08-29 · 09-14 오너 요청으로 되살림).**
  대신: 서버 cron 을 **추가·변경할 때는 반드시 상한(max_users·max_tickers)을 둔다.**
  사용자가 늘면 그 한 줄이 하루 청구서를 결정한다.
- **uvicorn `--workers` 를 늘리지 말 것.** `_demo_ai_calls`(데모 AI 일일 상한)와 `ttl_cache` 가
  프로세스 지역이라, 쪼개면 상한이 조용히 배가 되고 캐시 적중률이 반토막 나 외부 호출이
  오히려 는다. 인메모리 상태를 Redis 로 뺀 뒤에만 다시 검토.
- **등록 탭의 `savePortfolio` 를 한 건 추가에 쓰지 말 것.** 전체 교체라 관심목록까지
  덮어쓴다. 한 건은 `addHolding`.
- **화면에 실명(`user.name`)을 띄우지 말 것.** `displayNameOf()` 만 쓴다.

## ⚠️ 반복해서 밟은 함정 (같은 실수 방지)
0. **"앱이 느리다"에서 앱부터 의심하지 말 것.** 같은 요청을 **로컬(127.0.0.1) / 오리진 직접
   (--resolve 로 CF 우회) / 공개 주소** 세 지점에서 재면 어느 구간인지 한 번에 갈린다.
   2026-09-23 실측: 0.003s / 0.03s / 0.2~60s → 원인은 앱이 아니라 MTU 였다.
...
9. **오버레이를 새로 만들면 z-index 를 먼저 확인.** 하단바 9999 · 전체메뉴 시트 10000 ·
   InfoTip 10001 · Changelog 9999 가 이미 높다. 낮게 잡으면 막이 하단바를 못 덮고
   클릭까지 빼앗긴다(2026-08-21 투어에서 발생 → 20000).
10. **리팩터링에서 '빌드 통과' 는 아무것도 보장하지 않는다.** 2026-08-28 공용 모듈로
   옮기다가 문자열 경계를 잘못 잡아 `DaonAIReport` 등 **491줄이 지워졌는데도 빌드는
   성공**했다(JS 는 미정의 참조를 빌드 타임에 안 잡는다). `git diff --stat` 의 삭제
   줄 수를 반드시 확인하고, 블록을 잘라낼 때는 함수 개수 같은 불변식을 검증할 것.
11. **정규장 밖 시세를 그대로 '오늘 등락률' 로 쓰지 말 것.** 일봉의 마지막 값은
   프리장에서 '어제' 다. `session`·`ext` 필드를 함께 보고 라벨을 붙인다.
   평가액 계산의 기준은 계속 정규장 종가다(확장시간은 호가가 튄다).
12. **개인 데이터는 "읽는 경로가 지금 없다"로 안심하지 말 것.** 2026-08-26 사고에서
   개인 재무 리포트가 공유 캐시에 7행 있었는데, 조회가 정확한 키를 요구해 당장은
   안 샜다. 하지만 같은 파일에 접두 검색이 이미 있어 한 줄만 잘못 추가하면 터진다.
   **잘못된 자리에 있는 개인 데이터는 그 자체가 결함이다.**
13. **`currentUser` 를 effect 의존성에 넣을 때 주의.** authMe 응답마다 새 객체라 effect 가
   재실행되고, cleanup 에서 타이머를 지우면 예약이 매번 취소된다. 지연 실행은 ref 에 담고
   언마운트에서만 정리할 것(2026-08-21 투어가 아예 안 뜬 원인).

---

14. **JSX 에 훅을 추가하면 import 도 함께 추가했는지 본다.** 2026-09-14 `DiscoverTab` 에
   `useRef`/`useEffect` 를 쓰고 import 를 빠뜨려 **빌드는 통과, 런타임에 탭이 통째로 죽었다.**
   ErrorBoundary 가 그 탭만 격리해 다른 화면은 살았다. 배포 후 **그 탭을 실제로 연다.**
15. **Bash heredoc 은 따옴표로 막아도 백슬래시를 먹는다.** 파이썬 치환 안의 JS 문자열
   줄바꿈 이스케이프가 실제 줄바꿈이 되어 `Unterminated string literal`, 윈도우 경로의
   역슬래시가 유니코드 이스케이프로 해석되어 `truncated \UXXXXXXXX` 가 났다.
   **이스케이프가 들어가는 편집은 Write/Edit 도구로 조각을 만든 뒤 합친다.**
16. **자기 검사기의 판정을 먼저 의심한다.** 이번 주에는 검사기가 틀린 적이 앱이 틀린 것보다
   많았다 — 기간 최고치를 총액으로 비교 / 계좌 카드를 히어로로 읽음 / 구분선이 단축키 번호를
   먹음 / '세액공제 한도 소진'을 오류로 판정. 실패가 뜨면 **라벨과 요소부터** 확인한다.
17. **서버가 대신 채워 보내는 값을 확인한다.** 실명 노출을 프런트에서 막았는데 서버 세션이
   `nickname or name` 으로 실명을 실어 보내고 있었다. 오너 계정은 닉네임이 있어 고쳐진 것처럼
   보였다 — **대표 계정 하나로 검증하지 말 것.**

---

## ✅ 세션 (2026-09-09 ~ 09-15) — 지인 피드백 · 자가발전 루프 · 숫자 규약 · 발굴탭

상세 근거는 DEVELOPMENT_LOG 2026-09-09~15 항목. 커밋 `f0db12a` ~ `7ec9ebc`.

**발단**: 지인이 첫 사용 30분 만에 10건을 지적했다. 전부 맞는 말이었고 **전부 먼저
찾을 수 있었던 것**이었다 — 새 기술이 아니라 "처음 쓰는 사람의 눈"이 없었다.
그래서 두 짝의 루프를 만들었다: 바깥(논문·앱·기사) + 안(첫사용 UX·개인정보·숫자 기계 점검).

**가장 큰 사고 2건**
- **실명 노출** — 가입 폼이 '이름 / 홍길동'으로 실명을 받았고 화면이 그걸 띄웠다.
  동료도 자기 실명을 보고 놀랐다. 프런트만 고쳤을 때는 **안 고쳐진 상태**였다(함정 17).
- **탭별 총자산 불일치** — 604만원 차이. 계산이 아니라 이름 문제였다(숫자 규약).

**다음 세션 시작점**: 위 "남은 것" 1·2번. API 자동 충전을 켰는지부터 오너에게 확인.
