...
const FEATURES = [
  {
    ...
    details: [
      ...
      '종목 수정 → 수동 기준가: 시세가 조회되지 않는 한국 펀드·일부 ETF는 직접 입력한 기준가로 평가·비중·추이에 반영',
    ],
  },
  {
    key: 'allocation', title: '분석 탭',
    desc: 'Ⅰ스냅샷 · Ⅱ리스크 진단 · Ⅲ시계열 액션의 3단 구성 — 자산 현황부터 실행 전략까지 한 곳에서.',
    details: [
      '📈 자산 추이 (Net Worth): 일별 자동 누적 + 1M/3M/6M/1Y/ALL 토글',
      '🎯 목표 기반 포트폴리오: 목표금액·시점·월납입 입력 → 달성확률·80% 신뢰구간 + 목표 달성 필요 수익률(CAGR) + 행동 조언 ("산정 근거·방법론" 펼침)',
      '🏆 Portfolio Health Score: 경고 카드 위 한 줄 배지(점수·등급·가장 약한 축) — "자세히"를 누르면 4지표 설명이 펼쳐집니다',
      '🛎 자동 리밸런싱 경고: 단일 종목 30%+ / 섹터 50%+ / -20% 손실 / ETF+개별 중복 / 종목 ≤2개 — 심각도별 카드 + 임계값 조정. **계좌 단위로도 볼 수 있습니다**(기본은 전체 — 계좌 하나만 보면 분산이 실제보다 나빠 보입니다)',
      '🔗 종목 간 상관관계 매트릭스: 1년 일별 수익률 → Pearson 상관 히트맵 (낮을수록 분산 효과 ↑)',
      '💰 배당금 이력 & 캘린더: 최근 48개월 분기 히스토그램(YY/NQ) + 연간 예상 배당 + 다가오는 ex-date (현재 수량 기준 환산, 한국 종목 포함)',
      '🤖 AI 전략 리포트 (Claude Sonnet 4.6): 종합 리스크 진단 + 5년 단위 자산배분 + 목표 배당 시뮬(비중·배당률 직접 조정) + 추천 액션. 생성 약 1~3분(백그라운드 진행).',
      '📤 분석 리포트 MD 내보내기: 보유구성·Health·경고·배당·AI전략을 마크다운으로 — 타 LLM 교차검증용',
    ],
  },
  {
  ...
  {
    key: 'discover', title: '발굴 탭',
    desc: '성장하면서도 저평가된 신규 종목을 5요소로 점수화해 매일 자동 발굴합니다 (GARP 전략).',
    details: [
      '5요소 점수: 저평가(PEG·선행PER)·성장성(매출·EPS)·안정성(ROE·부채)·상승세(52주 고점 근접)·기대(애널리스트 목표가·추정치 상향)',
      '게이트: PEG≤1.5 · EPS성장>0 · 부채<200% · 매출 급감 제외 — 기준 통과 종목만 추천',
      '행 클릭 → 5요소 레이더 + 현재가·목표가·상승여력·PER·선행PER·PEG·순이익률 + 거래소·국가·유형',
      '미국·한국 모두 지원 (점수는 같은 시장 내 상대 순위·섹터 중립). 매일 장 마감 후 자동 갱신',
      '종목 클릭 → AI 심층 분석 연결. 투자 권유가 아닌 1차 스크리닝 도구 (정직성: 빈 지표는 가짜값 없이 N/A)',
    ],
  },
  {
  ...
  },
  {
    ...
    details: [
      ...
      '테마: 화이트 · 다크(네이비) · 차콜(중성 회색) · 자동(OS 따라감) — 설정 탭에서 선택',
    ],
  },
  {
    key: 'admin', title: '관리자 탭 (admin 전용)',
