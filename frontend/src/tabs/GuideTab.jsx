...
const FEATURES = [
  ...
  },
  {
    ...
    details: [
      ...
      '🏆 Portfolio Health Score: 0-100점 + S/A/B/C/D 등급 + 4지표 설명 + 약점 강조 (외부 지표 불가 시 추정 표기)',
      '🛎 자동 리밸런싱 경고(리스크 진단 카드): 단일 종목 30%+ / 섹터 50%+ / -20% 손실 / ETF+개별 중복 / 종목 ≤2개 — 심각도별(CRITICAL/MEDIUM) 카드 + 임계값 조정',
      ...
      '🤖 AI 전략 리포트 (Claude Sonnet 4.6): 5년 단위 자산배분·리밸런싱·액션 + 월 배당 전환 시뮬(비중·배당률 직접 조정) + 저점발굴 시계열 매칭. 생성 약 1~3분(백그라운드 진행).',
